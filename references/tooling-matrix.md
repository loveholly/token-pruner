...
| `yq` | YAML/TOML/XML field selection | Core for config-heavy flows | k8s manifests, CI configs, Helm values, TOML configs | Converts YAML to JSON for downstream pruning. Also handles TOML and XML |
...
| `tiktoken` | Token measurement | Optional | Before/after token counting for cl100k_base | Falls back to byte_count/4 estimate when unavailable |
| Python stdlib | Orchestration | Core dependency | Tool probing, heuristics, sampling, vendored tool routing, ANSI stripping, output truncation | Keeps the skill self-contained |
...
2. Optional fallback:
   - `gojq`
   - `tiktoken` (Python package, for exact token measurement)
...
- YAML or TOML:
  - Select fields with `yq` first.
  - Convert to compact JSON for downstream processing.
- CSV or TSV:
  - Prune with `qsv`.
...
- Oversized output (any source):
  - Strip ANSI escapes.
  - Truncate to head + tail with a summary line.
  - For Claude Code this happens automatically via PostToolUse hook.
  - For other agents, pipe through `truncate` explicitly.
